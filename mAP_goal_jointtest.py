...
class QueryLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data = self.processor(images=image, text=caption, return_tensors="pt", truncation=True, padding="max_length", max_length=self.new_max_token)
...
def main(args):
    ...
    if args.dataset == 'docci':
        ...
        query_list = 'datasets/DCI_test_joint_sim_max_1_1.json'

    with open(query_list, encoding='utf-8') as f:
        query_list = json.loads(f.read())

    args.query_list = query_list

    query_dataset = QueryLoader(query_list, processor, args.new_max_token)
