...
def main(args):
    ...
    train(fabric, model, optimizer, train_loader, processor, args.resume_epoch)

def train(fabric: L.Fabric, model: torch.nn.Module, optimizer: torch.optim.Optimizer, train_loader, processor, resume_epoch=0) -> None:
    start_iter = resume_epoch * len(train_loader)
    iter = start_iter
...
def get_args_parser():
    ...
    parser.add_argument('--resume_epoch', type=int, default=0,
                        help='Resume from this epoch (loads checkpoint from this epoch and continues from next)')
